...
Keeps the Render-hosted app (`https://stravagancia.onrender.com`) awake during the
day so it isn't asleep when you open it.
...
Worker **`strava-keepalive`** (`https://strava-keepalive.maxicombina.workers.dev/`)
with a **Cron Trigger `*/12 * * * *`** (every 12 min).
...
Edit code → Deploy). This file is the source of record — keep them in sync. The
Cron Trigger lives under the Worker's **Settings → Triggers → Cron Triggers**.
...
> History: earlier attempts used cron-job.org (backed off), a self-hosted Docker
> pinger on an Oracle Cloud Always Free ARM VM (perpetual "out of host capacity"),
> and GCP e2-micro (compute free but the in-use external IPv4 costs ~$3.65/mo, so
> not $0). Cloudflare Workers won on cost, reliability, and zero maintenance.
